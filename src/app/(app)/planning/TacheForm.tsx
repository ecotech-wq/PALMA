...
export function CreateTacheForm({
  ...
  // Entrée discrète dans la rangée d'outils du planning ; le formulaire
  // complet se déplie SOUS la rangée (order-last + w-full dans le
  // conteneur flex-wrap parent), comme la barre de saisie rapide.
  return (
    <>
      <button
        type="button"
        ...
        className={cn(
          "inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs rounded-md transition-colors shrink-0",
          open
            ? "bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-slate-100"
            : "text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100 hover:bg-slate-100 dark:hover:bg-slate-800"
        )}
      >
        <FilePlus2 size={14} className="shrink-0" /> Formulaire
      </button>
      {open && (
        <div className="order-last w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 space-y-3">
